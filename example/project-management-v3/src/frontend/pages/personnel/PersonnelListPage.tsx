    ...
    <div className="flex items-center gap-3 relative z-10">
      <div className={cn(
        ...
        color === 'blue' ? 'bg-primary/10 text-primary' :
          color === 'emerald' ? 'bg-accent/10 text-accent' :
            color === 'amber' ? 'bg-amber-50 text-amber-600' : 'bg-indigo-50 text-indigo-600'
      )}>
        <Icon size={18} strokeWidth={2.5} />
      </div>
      <div>
        <p className="text-[10px] font-bold text-slate-400 mb-0.5">{title}</p>
...
export default function PersonnelListPage() {
  ...
  const loadPositions = async () => {
    try {
      const res = await apiClient.get<any>(API_URL.DATA('Position'), { params: { pageSize: 1000 } })
      setPositions(res.data || res.items || res || [])
    } catch (e) { }
  }

  const loadDepartments = async () => {
    try {
      const res = await apiClient.get<any>(API_URL.DATA('Department'), { params: { pageSize: 1000 } })
      setDepartments(res.data || res.items || res || [])
    } catch (e) { }
  }
  ...
  const loadEmployees = async () => {
    try {
      setLoading(true)
      const res = await apiClient.get<any>(API_URL.DATA('Employee'), {
        params: { page, pageSize: 12, ...(searchTerm && { search: searchTerm }) }
      })
      if (res) {
        setEmployees(res.data || res.items || res || [])
  ...
  const handleDelete = async (employee: Employee) => {
    ...
    try {
      await apiClient.delete(`${API_URL.DATA('Employee')}/${employee.id}`)
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await apiClient.put(`${API_URL.DATA('Employee')}/${editingEmployee.id}`, {
    ...
    <div className="min-h-screen bg-mesh p-4 lg:p-6 space-y-4 animate-fade-in custom-scrollbar">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <motion.div initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
          <h1 className="text-2xl font-bold text-slate-700 flex items-center gap-4">
            <div className="p-2.5 bg-primary text-white rounded-xl shadow-brand">
              <User size={24} strokeWidth={2.5} />
            </div>
            {t('personnel.list_title')}
          </h1>
          <p className="text-secondary font-bold text-sm mt-1.5 opacity-70 italic">{t('personnel.list_subtitle')}</p>
        </motion.div>
      ...
      <div className="premium-card p-4 flex flex-wrap items-center gap-4">
        ...
        <button
          ...
          className="p-2.5 bg-white rounded-xl border border-slate-200 text-slate-400 hover:text-primary transition-all shadow-sm"
        >
                ...
                >
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-medium text-slate-600 truncate">{getDepartmentName(emp.department_id)}</span>
                    </div>
                  </div>
