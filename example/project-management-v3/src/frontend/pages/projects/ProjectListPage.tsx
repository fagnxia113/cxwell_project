      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-4 mb-2">
            ...
            <h1 className="text-2xl font-bold text-slate-700">{t('project.list_title')}</h1>
          </div>
          <p className="text-secondary font-bold text-sm mt-1.5 opacity-70 italic">{t('project.fields.list_subtitle')}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-inner">
            <button
              ...
              className={cn("p-2 rounded-lg transition-all", viewMode === 'table' ? "bg-white text-primary shadow-sm" : "text-slate-400 hover:text-slate-600")}
            >
              <ListIcon size={16} />
            </button>
            <button
              onClick={() => setViewMode('grid')}
              className={cn("p-2 rounded-lg transition-all", viewMode === 'grid' ? "bg-white text-primary shadow-sm" : "text-slate-400 hover:text-slate-600")}
            >
              <LayoutGrid size={16} />
            </button>
          </div>
          ...
          <HasPermission code="project:create">
            ...
            >
              <Upload size={18} /> {t('project.action.import')}
            </button>
          </HasPermission>
          ...
          >
            <Plus size={18} strokeWidth={2.5} /> {t('project.action.initiate')}
          </PermissionButton>
        </div>
      </div>
