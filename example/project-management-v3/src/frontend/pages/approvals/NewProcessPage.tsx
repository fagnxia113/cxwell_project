...
export default function NewProcessPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [presetsData, definitionsData] = await Promise.all([
        apiClient.get<any>(API_URL.WORKFLOW.PROCESS_PRESETS).catch(() => null),
        apiClient.get<any>(`${API_URL.BASE}/api/workflow/definitions?status=active&pageSize=100`).catch(() => null)
      ]);
      setPresets(Array.isArray(presetsData) ? presetsData : (presetsData?.data || []));
      setWorkflowDefinitions(Array.isArray(definitionsData) ? definitionsData : (definitionsData?.data || []));
    } catch (e) { console.error('加载流程数据失败:', e) } finally { setLoading(false) }
  }
  ...
  const categories = useMemo(() => {
    const keys = new Set([...presets.map(p => p.category), ...workflowDefinitions.map(d => d.category || 'general')])
