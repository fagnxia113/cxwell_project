        ...
        {projects.map((project, i) => {
          ...
          return (
            <motion.div 
              ...
              className="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 cursor-pointer flex flex-col relative overflow-hidden h-full"
            ...
            >
              ...
              <div className="p-6 pb-2">
                ...
                <h3 className="text-lg font-black text-slate-900 group-hover:text-blue-600 transition-colors mb-1 line-clamp-1 tracking-tight uppercase">{project.name}</h3>
                <div className="text-[10px] font-bold text-slate-400 font-mono uppercase tracking-widest flex items-center gap-1.5 opacity-60">
              ...
              <div className="p-6 space-y-4 flex-1">
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] font-black text-blue-600 tabular-nums">{project.progress}%</span>
                  </div>
                  <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden shadow-inner uppercase tracking-wider">
                    <motion.div 
                      initial={{ width: 0 }}
              ...
              <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-50 grid grid-cols-3 gap-2">
                <div className="flex flex-col gap-0.5">
                  <div className="flex items-center gap-1.5 text-slate-400">
                    <Zap size={10} />
                    <span className="text-[8px] font-black uppercase tracking-widest">IT Cap</span>
                  </div>
                  <span className="text-[11px] font-black text-slate-700 tabular-nums">{project.it_capacity || '-'}</span>
                </div>
                <div className="flex flex-col gap-0.5 border-x border-slate-100 px-2 text-center">
                  ...
                  <span className="text-[11px] font-black text-slate-700 tabular-nums">{project.cabinet_count || '-'}</span>
                </div>
                <div className="flex flex-col gap-0.5 text-right">
                  ...
                  <span className="text-[11px] font-black text-slate-700 tabular-nums">{project.building_area || '-'}</span>
                </div>
              </div>

              {/* Footer */}
              <div className="p-4 px-6 flex justify-between items-center group-hover:bg-slate-50 transition-colors">
