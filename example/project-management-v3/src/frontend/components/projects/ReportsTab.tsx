...
export default function ReportsTab({ projectId, milestones }: ReportsTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-400">{t('common.loading') || '加载中...'}</div>
      ) : groupedReports.every(g => g.reports.length === 0) ? (
        <div className="text-center py-12 bg-white rounded-xl border border-slate-100">
          <FileText size={48} className="mx-auto text-slate-300 mb-3" />
          <p className="text-sm text-slate-400">{t('project.reports.empty') || '暂无报告，点击添加按钮创建'}</p>
        </div>
      ) : (
        <div className="space-y-6">
          {groupedReports.filter(g => g.reports.length > 0).map(({ milestone, reports: msReports }) => (
            <div key={milestone.id} className="bg-white rounded-xl border border-slate-100 overflow-hidden">
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${milestone.status === 'completed' ? 'bg-emerald-500' : milestone.status === 'in_progress' ? 'bg-blue-500' : 'bg-slate-300'}`} />
                <span className="text-xs font-bold text-slate-700">{milestone.name}</span>
                <span className="text-xs text-slate-400">({msReports.length} {t('project.reports.items') || '项'})</span>
              </div>
                ...
                {msReports.map(report => {
                  ...
                  return (
                    <div key={report.id} className="px-4 py-3 flex items-center gap-4 hover:bg-slate-50 transition-colors">
                        ...
                        {report.attachments && report.attachments.length > 0 && (
                          <div className="mt-2 space-y-1">
                            {report.attachments.map(att => (
                              <div key={att.id} className="flex items-center justify-between group/att bg-slate-50 px-2 py-1 rounded border border-slate-100 max-w-sm">
                                <a 
                                  href={att.file_url} 
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                  className="text-xs text-emerald-600 flex items-center gap-1 hover:underline truncate"
                                >
                                  <File size={10} />
                                  {att.file_name}
                                </a>
                      ...
                      <div className={`px-2 py-1 rounded-lg text-xs font-bold ${config.bgColor} ${config.color}`}>
                        {config.label}
                      </div>
