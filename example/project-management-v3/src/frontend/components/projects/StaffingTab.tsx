    ...
    <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xl space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('daily_report.select_personnel')}</label>
              <select 
                value={newPlan.employee_id}
                onChange={e => setNewPlan({...newPlan, employee_id: e.target.value})}
                className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-600 outline-none transition-all"
              >
                {personnel.map(p => (
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('project.staffing.plan_type')}</label>
              <select 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('project.staffing.start_date')}</label>
              <input 
                type="date" 
                value={newPlan.start_date}
                onChange={e => setNewPlan({...newPlan, start_date: e.target.value})}
                className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-bold focus:ring-2 focus:ring-indigo-600 outline-none transition-all"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('project.staffing.end_date')}</label>
              <input 
                type="date" 
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('project.staffing.notes')}</label>
            <input 
              type="text" 
              value={newPlan.notes}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={handleAdd} className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-black shadow-lg hover:bg-indigo-700 transition-all">{t('common.submit')}</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {staffingPlans.length === 0 ? (
          <div className="md:col-span-2 py-20 text-center bg-white rounded-3xl border border-slate-100 border-dashed">
            <p className="text-slate-300 font-bold uppercase tracking-widest text-xs">{t('project.staffing.no_plans')}</p>
          </div>
        ) : (
          staffingPlans.map(plan => (
            <div key={plan.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    {plan.plan_type === 'rotation' ? <Repeat size={18} /> : <Coffee size={18} />}
                  </div>
                  <div>
                    <h4 className="text-sm font-black text-slate-900 uppercase tracking-tight">{plan.employee_name || 'Project Staff'}</h4>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl space-y-3">
                ...
                {plan.notes && (
                  <div className="pt-2 border-t border-slate-200/50">
                    <p className="text-[11px] font-bold text-slate-500 italic">"{plan.notes}"</p>
                  </div>
                )}
              </div>
            </div>
