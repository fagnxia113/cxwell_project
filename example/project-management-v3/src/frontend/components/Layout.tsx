...
const getSubPathName = (parentPath: string | undefined, subPath: string | undefined, t: any): string => {
  ...
  const subPathMap: Record<string, Record<string, string>> = {
    'equipment': {
      'list': t('sidebar.ledger'),
      'statistics': t('sidebar.monitoring'),
    },
    'projects': {
...
const getMenus = (t: any): MenuItem[] => [
  ...
  // 3. 审批中心
  {
    ...
    children: [
      { key: 'wf-new', label: t('sidebar.startApproval'), path: '/approvals/new', permission: 'menu:workflow' },
      { key: 'wf-pending', label: t('sidebar.pending'), path: '/approvals/pending', permission: 'menu:workflow' },
      { key: 'wf-completed', label: t('sidebar.completed'), path: '/approvals/completed', permission: 'menu:workflow' },
      { key: 'wf-mine', label: t('sidebar.myInitiated'), path: '/approvals/mine', permission: 'menu:workflow' },
      { key: 'wf-draft', label: t('sidebar.drafts'), path: '/approvals/draft', permission: 'menu:workflow' },
    ]
  },
  ...
  // 5. 设备管理
  {
    ...
    icon: 'box',
    permission: 'menu:equipment',
    children: [
      { key: 'equip-ledger', label: t('sidebar.ledger'), path: '/equipment', permission: 'menu:equipment' },
      { key: 'equip-acc', label: t('sidebar.accessories'), path: '/equipment/accessories', permission: 'menu:equipment' },
      { key: 'equip-hist', label: t('sidebar.history'), path: '/equipment/history', permission: 'menu:equipment' },
      { key: 'warehouse', label: t('sidebar.warehouse'), path: '/warehouses', permission: 'menu:equipment' },
    ]
  },
