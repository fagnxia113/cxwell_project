...
router.get('/:entity', async (req: Request, res: Response) => {
  try {
    ...
    if (!entityDef) {
      return res.status(404).json({ error: `实体 ${entityName} 不存在` });
    }

    const tableName = metadataService.getTableName(entityName);
    ...
    if (entityName === 'Employee') {
      data = await db.query(
        `SELECT e.*, d.name as department_name 
         ...
         WHERE ${whereClause.replace(/`department_id`/g, 'e.`department_id`')} 
         ${orderClause} LIMIT ${pageSizeNum} OFFSET ${offset}`,
        params
      );
    } else if (entityName === 'ProjectPersonnel' || entityName === 'project_personnel') {
      data = await db.query(
        `SELECT pp.*, e.name as employee_name 
         FROM \`${tableName}\` pp 
         LEFT JOIN employees e ON pp.employee_id = e.id 
         WHERE ${whereClause.replace(/`employee_id`/g, 'pp.`employee_id`')} 
         ${orderClause} LIMIT ${pageSizeNum} OFFSET ${offset}`,
        params
      );
    } else {
...
router.get('/:entity/:id', async (req: Request, res: Response) => {
  try {
    ...
    const entityDef = metadataService.getEntity(entityName);
    if (!entityDef) {
      return res.status(404).json({ success: false, error: `实体 ${entityName} 不存在` });
    }

    const tableName = metadataService.getTableName(entityName);
    const pkField = metadataService.getPrimaryKey(entityName);
...
router.post('/:entity', async (req: Request, res: Response) => {
  try {
    ...
    // 处理自动生成字段
    data = await metadataService.processAutoFields(entityName, data);

    // 处理时间戳
    data = metadataService.addTimestampFields(entityName, data, false);

    // 特殊处理：任务WBS编号自动生成
    if (entityName === 'Task' || entityName === 'tasks') {
      if (!data.wbs_code && data.project_id) {
        const taskUseCase = container.resolve(TaskUseCase);
        data.wbs_code = await taskUseCase.generateWBSCode(
          data.project_id,
          data.parent_id || null
        );
      }
...
router.put('/:entity/:id', async (req: Request, res: Response) => {
  try {
    ...
    // 处理时间戳
    data = metadataService.addTimestampFields(entityName, data, true);
...
router.delete('/:entity/:id', async (req: Request, res: Response) => {
  try {
    ...
    const entityDef = metadataService.getEntity(entity as string);
    if (!entityDef) {
