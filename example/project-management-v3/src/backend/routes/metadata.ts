...
/**
 * 获取枚举选项（用于下拉框）
 * GET /api/metadata/enum-options/:enumName
 */
router.get('/enum-options/:enumName', (req: Request, res: Response) => {
  try {
    const { enumName } = req.params;
    const enumNameStr = Array.isArray(enumName) ? enumName[0] : enumName;
    const enumDef = metadataService.getEnum(enumNameStr);
    ...
    const options = enumDef.values.map(v => ({ label: v.label, value: v.value }));
    res.json(options);
  } catch (error) {
    console.error('获取枚举选项失败:', error);
    res.status(500).json({ error: '获取枚举选项失败', message: error });
  }
});
