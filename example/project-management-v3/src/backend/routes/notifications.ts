...
router.post('/alerts/:id/acknowledge', async (req: Request, res: Response) => {
  try {
    ...
    await prisma.progress_alerts.update({
      ...
      data: {
        status: 'acknowledged',
        acknowledged_by: acknowledged_by as string,
...
router.post('/daily-reports/remind', async (req: Request, res: Response) => {
  try {
    const result = await dailyReportReminderService.sendDailyReportReminder(req.body.project_id, req.body.person_id)
...
router.put('/purchase/requests/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    await purchaseReminderService.updatePurchaseRequestStatus(req.params.id as string, status as any, notes)
...
router.post('/read-all', async (req: Request, res: Response) => {
  try {
    const userId = (req.user!.id || req.body.user_id) as string
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const notification = await notificationRepository.findById(req.params.id as string)
    if (!notification) {
