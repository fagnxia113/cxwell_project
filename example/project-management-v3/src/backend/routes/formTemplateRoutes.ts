...
router.get('/key/:key', async (req, res) => {
  try {
    const template = await formTemplateService.getTemplateByKey(req.params.key);
    if (!template) {
    ...
    console.error('通过Key获取表单模板失败:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : '通过Key获取表单模板失败'
    });
  }
});
