...
async function ensureUsersTable() {
  ...
  if (!count || count.count === 0) {
    ...
    `, [hashedPassword])
    logger.info(`默认管理员已初始化，请尽快登录并修改默认密码。初始密码为: ${defaultPassword}`)
  }
}
...
router.get('/verify', asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({ 
    success: true, 
    data: { ...user, employee_id: employee?.id || null } 
  })
}))
...
router.delete('/users/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await db.queryOne<{ username: string; role: string }>('SELECT username, role FROM users WHERE id = ?', [id])
  if (!user) {
    throw new ValidationError('用户不存在')
  }

  if (user.username === 'admin') {
    throw new ValidationError('系统主管理员账号不能删除')
  }

  // 检查是否是最后一个管理员
  if (user.role === 'admin') {
    const adminCount = await db.queryOne<{ count: number }>('SELECT COUNT(*) as count FROM users WHERE role = "admin"')
    if (adminCount && adminCount.count <= 1) {
      throw new ValidationError('系统中必须保留至少一个管理员账号')
    }
  }
  ...
  await db.transaction(async (connection) => {
    ...
    // 3. 处理工作流任务 - 物理删除未开始的任务，保留已完成/进行中的记录但清除关联
    ...
    // 4. 清理流程定义中的审批人配置 (谨慎操作)
    const [definitions]: any = await connection.query(
    ...
    for (const def of definitions) {
      ...
      try {
        ...
        if (nodeConfig && Array.isArray(nodeConfig.nodes)) {
          nodeConfig.nodes.forEach((node: any) => {
            if (node.config?.approverSource?.type === 'user') {
              const userIds: string[] = node.config.approverSource.value ? node.config.approverSource.value.split(',') : []
              const filteredIds = userIds.filter((uid: string) => uid !== id)
              
              if (filteredIds.length !== userIds.length) {
                changed = true
                node.config.approverSource.value = filteredIds.join(',')
                // 如果过滤后没审批人了，改为可选或保持空字符串待配置
              }
            }
          })
        }
        ...
        logger.error(`解析流程定义 ${def.id} 失败，跳过其用户清理`, e)
      }
    }
  })
