...
router.get('/definitions/key/:key', async (req: Request, res: Response) => {
  try {
    ...
    // 如果是以 preset- 开头，先尝试通过集成的 preset 查找对应的 workflowTemplateId
    if (key.startsWith('preset-')) {
      const preset = processFormIntegrationService.getPresetById(key);
      if (preset) {
        key = preset.workflowTemplateId;
      }
    }
    
    let definition = await definitionService.getDefinitionByKey(key as string);
    
    if (!definition) {
      // 如果还没找到，尝试去掉 preset- 前缀再找一次 (兜底逻辑)
      if (key.startsWith('preset-')) {
        definition = await definitionService.getDefinitionByKey(key.replace('preset-', ''));
      }
    }
...
router.get('/definitions/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const definition = await definitionService.getDefinition(id as string);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info('转换后的节点配置', { 
        ...
        allNodes: node_config.nodes.map((n: any) => ({ id: n.id, name: n.name, config: n.config }))
      });
      
    const success = await definitionService.updateDefinition(id as string, {
        key,
        name,
        category,
        entity_type,
      ...
      // 获取更新后的定义
      const updated = await definitionService.getDefinition(id as string);
...
router.post('/definitions/:id/activate',
  ...
  try {
    const { id } = req.params;
    const result = await definitionService.activateDefinition(id as string);
...
router.post('/definitions/:key/reload',
  ...
  try {
    ...
    const existingDefinition = await definitionService.getLatestDefinition(key as string);
 ...
 */
router.post('/definitions/:id/suspend',
  requireAdmin,
  auditLog('suspend_definition'),
  async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await definitionService.suspendDefinition(id as string);

    if (!result) {
      return res.status(404).json({ error: '流程定义不存在' });
    }

    res.json({ success: true, message: '流程定义已挂起' });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
...
router.delete('/definitions/:id',
  ...
  try {
    const { id } = req.params;
    const result = await definitionService.deleteDefinition(id as string);
...
router.post('/processes',
  ...
  async (req: Request, res: Response) => {
  try {
    const { processKey, definitionKey, businessKey, businessId, title, variables, initiator } = req.body;

    const instance = await workflowEngine.startProcess({
      processKey: processKey || definitionKey,
      businessKey,
      businessId,
      title,
      variables,
...
router.get('/processes', async (req: Request, res: Response) => {
  try {
    ...
    const instancesResult = await instanceService.getInstances({
    ...
    const instancesWithCurrentNode = await Promise.all(
      instancesResult.data.map(async (instance) => {
        // 如果实例已完成，根据结果确定节点名称
        ...
        let currentNodeName = instance.current_node_name || '未知';
...
router.get('/processes/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const instance = await workflowEngine.getProcessInstance(id as string);
...
router.get('/processes/business/:businessId', async (req: Request, res: Response) => {
  try {
    ...
    if (instances && instances.data && instances.data.length > 0) {
      const instance = await workflowEngine.getProcessInstance(instances.data[0].id);
      res.json({ success: true, data: instance });
    } else {
...
router.post('/processes/:id/terminate',
  ...
  try {
    ...
    await workflowEngine.terminateProcess(id as string, reason, { id: req.user!.id, name: req.user!.name });
...
router.post('/processes/:id/withdraw',
  ...
  try {
    ...
    // 获取流程实例
    const variables = await instanceService.getInstanceVariables(id as string);
    const instance = await instanceService.getInstance(String(id));
    if (!instance) {
      return res.status(404).json({ error: '流程实例不存在' });
    }
    ...
    // 结束流程实例，标记为已撤回
    await instanceService.endInstance(String(id), 'withdrawn', operator || { id: 'system', name: '系统' }, reason);

    // 取消所有待处理的任务
    // 取消所有待处理的任务
    const tasks = await taskService.getTasksByInstance(String(id));
...
router.get('/my-tasks', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const tasks = await workflowEngine.getTasksByAssignee(userId);

    let filteredTasks = tasks;
    if (status) {
      const statusArray = Array.isArray(status) ? status.map(String) : [String(status)];
...
/**
 * 获取我的待办任务数量
 * GET /api/workflow/my-pending-tasks/count
 */
router.get('/my-pending-tasks/count', authenticate, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const count = await taskService.getPendingTasksCount(userId);
    ...
    const completedTasksWithDetails = await Promise.all(
      completedTasks.map(async (task) => {
        ...
        return {
          ...
          action: (taskHistory as any)?.action || 'approved',
          comment: (taskHistory as any)?.comment || '',
          completed_at: task.completed_at || (task as any).updated_at,
...
router.get('/tasks', async (req: Request, res: Response) => {
  try {
    ...
    let tasks;
    if (instanceId) {
      tasks = await workflowEngine.getTasksByInstance(String(instanceId));
    } else if (assigneeId) {
      tasks = await workflowEngine.getTasksByAssignee(String(assigneeId));
    } else {
...
router.post('/tasks/:id/complete',
  ...
  try {
    ...
    await workflowEngine.completeTask(String(id), {
      action,
      comment,
      variables,
      operator
...
router.post('/tasks/:id/claim',
  ...
  try {
    ...
    await taskService.claimTask(String(id), userId, userName);
...
router.post('/tasks/:id/delegate',
  ...
  try {
    ...
    await workflowEngine.delegateTask(String(id), {
      targetUser,
      comment,
      operator
    });
...
router.post('/tasks/:id/transfer',
  ...
  try {
    ...
    await workflowEngine.transferTask(String(id), {
      targetUser,
      comment,
      operator
    });
...
router.get('/metrics', async (req: Request, res: Response) => {
  try {
    ...
    const metrics = await workflowEngine.getProcessMetrics(processKey ? String(processKey) : undefined);
...
router.get('/form-presets', async (req: Request, res: Response) => {
  try {
    ...
    let presets;
    if (category) {
      presets = processFormIntegrationService.getPresetsByCategory(String(category));
    } else if (businessType) {
      presets = processFormIntegrationService.getPresetsByBusinessType(String(businessType));
    } else {
...
router.get('/form-presets/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const preset = processFormIntegrationService.getPresetById(String(id));
...
router.put('/form-presets/:id', async (req: Request, res: Response) => {
  try {
    ...
    const success = processFormIntegrationService.updatePreset(String(id), req.body);
...
router.delete('/form-presets/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const success = processFormIntegrationService.deletePreset(String(id));
...
router.post('/form-presets/:id/activate', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const success = processFormIntegrationService.activatePreset(String(id));

    if (!success) {
      return res.status(404).json({ error: '流程表单预设不存在' });
    }

    res.json({ success: true, data: success, message: '流程表单预设已激活' });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/form-presets/:id/deactivate', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const preset = processFormIntegrationService.deactivatePreset(String(id));
...
router.get('/form-presets/:id/default-values', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const defaultValues = processFormIntegrationService.getFormDefaultValues(String(id));
...
router.get('/form-presets/:id/form-fields', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const formFields = await processFormIntegrationService.getFormFields(String(id));
...
router.post('/form-presets/:id/sync', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    await processFormIntegrationService.syncProcessFormTemplates();
    res.json({ success: true, message: '表单模板同步成功' });
  } catch (error: any) {
...
router.get('/form-templates', async (req: Request, res: Response) => {
  try {
    ...
    let templates;
    if (module) {
    templates = await unifiedFormService.getTemplatesByModule(String(module) as any);
    } else {
...
router.get('/form-templates/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const template = unifiedFormService.getTemplateById(String(id));
...
router.get('/form-templates/key/:key', async (req: Request, res: Response) => {
  try {
    const { key } = req.params;
    const template = await unifiedFormService.getTemplateByKey(String(key));
    ...
    if (itemsField) {
      let itemFields: any[] = [];
      if (itemsField.type === 'array' && (itemsField as any).arrayConfig) {
        itemFields = (itemsField as any).arrayConfig.fields;
      } else if (itemsField.type === 'array' && (itemsField as any).itemFields) {
        itemFields = (itemsField as any).itemFields;
      }
...
router.put('/form-templates/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const updates = req.body;
    const result = unifiedFormService.updateTemplate(String(id), req.body);
...
router.delete('/form-templates/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = unifiedFormService.deleteTemplate(String(id));
...
router.post('/form-templates/:id/validate', async (req: Request, res: Response) => {
  try {
    ...
    const result = await unifiedFormService.validateForm(String(id), formData);
    res.json({ success: true, data: result });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.post('/form-templates/:id/link', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { formData } = req.body;
    const result = await unifiedFormService.handleBusinessLink(String(id), formData);
