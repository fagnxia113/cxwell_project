...
// 注意：Request.user 类型已在 authMiddleware.ts 中统一定义，此处不重复声明
...
export const validateOperator = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.user && operator.id !== req.user.id) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    res.json = function(data: any) {
      if (req.user && (data.success || res.statusCode < 400)) {
        executionLogger.log({
          executionId: String(req.params.id || req.params.taskId || 'unknown'),
          action,
          instanceId: req.params.id as string,
          taskId: req.params.taskId as string,
          operator: { id: req.user.id, name: req.user.name || req.user.username },
          result: 'success',
          metadata: {
            method: req.method,
            path: req.path,
