...
export class TaskService {
  private orgCache: {
    data: { deptMap: any; posMap: any; userMap: any };
    timestamp: number;
  } | null = null;
  ...
  async createTask(params: {
    ...
    await db.insert(
      `INSERT INTO workflow_tasks (
        ...
        variables, status, approval_mode, vote_threshold, delegated_from, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        task.id,
        task.instance_id,
        ...
        task.candidate_users ? JSON.stringify(task.candidate_users) : null,
        task.candidate_groups ? JSON.stringify(task.candidate_groups) : null,
        task.priority ?? 50,
        task.due_date ?? null,
        null,
        task.variables ? JSON.stringify(task.variables) : null,
        task.status,
        task.approval_mode ?? 'or_sign',
        task.vote_threshold ?? 1,
        task.delegated_from ?? null,
        task.created_at
      ]
    );
  ...
  async getTasksByInstance(instanceId: string, status?: string[]): Promise<WorkflowTask[]> {
    ...
    const rows = await db.query<any>(
      `SELECT t.*, i.title as process_title, i.definition_key, i.initiator_id, i.initiator_name,
              e.user_id as assignee_user_id, d.form_template_id
    ...
    return rows.map((row: any) => {
      ...
      (task as any).form_template_id = row.form_template_id;
      return task;
    });
  }

  private async getOrgMaps() {
    const now = Date.now();
    if (this.orgCache && (now - this.orgCache.timestamp < this.CACHE_TTL)) {
      return this.orgCache.data;
    }

    try {
      const [departments, positions, employeeList] = await Promise.all([
        db.query<any>('SELECT id, name FROM departments'),
        db.query<any>('SELECT id, name FROM positions'),
        db.query<any>('SELECT id, name FROM employees')
      ]);
      ...
      this.orgCache = {
        data: { deptMap, posMap, userMap },
        timestamp: now
      };
    } catch (error: any) {
      console.error('[TaskService] getOrgMaps failed:', error);
      throw error;
    }
    return this.orgCache.data;
  }
  ...
  async getTasksByAssignee(assigneeId: string, status?: string[]): Promise<WorkflowTask[]> {
    ...
    const { deptMap, posMap, userMap } = await this.getOrgMaps();
    ...
    return rows.map((row: any) => {
      try {
        ...
        // 获取表单数据并转换ID为名称
        const formData = (instanceVars as any).formData || (task.variables as any)?.formData || {};
        ...
        console.error('[TaskService] Error processing task row:', row.id || 'unknown', error);
        return null;
      }
  ...
  async getPendingTasksCount(userId: string): Promise<number> {
    ...
    whereClause += ')';
    whereClause += ` AND t.status IN ('created', 'assigned', 'in_progress')`;

    const result = await db.queryOne<{ count: number }>(
      `SELECT COUNT(*) as count FROM workflow_tasks t WHERE ${whereClause}`,
      params
    );
      ...
      await db.transaction(async (conn) => {
        await conn.execute(
          `UPDATE workflow_tasks SET 
            ...
            duration = ? 
          WHERE id = ?`,
          [result, params.comment ?? null, now, duration ?? null, taskId]
        );

        // 记录任务完成历史
        await this.recordTaskHistoryWithConn(conn, taskId, {
          action: 'complete',
        ...
        if (params.formData) {
          ...
          if (instance) {
            const currentFormData = (instance.variables as any)?.formData || {};
            const mergedFormData = { ...currentFormData, ...params.formData };
            variablesToUpdate.formData = mergedFormData;
          }
        }
        
        if (Object.keys(variablesToUpdate).length > 0) {
          await instanceService.updateVariablesWithConn(conn, task.instance_id, variablesToUpdate);
        }
      });
    });
  }

  private async recordTaskHistoryWithConn(conn: any, taskId: string, params: any, instanceId?: string, nodeId?: string): Promise<void> {
    const task = await this.getTask(taskId);
    await conn.execute(
  ...
  async delegateTask(taskId: string, targetUser: { id: string; name: string }, operator: { id: string; name: string }, comment?: string): Promise<void> {
    return taskLockManager.delegateTask(taskId, operator.id, async () => {
      const task = await this.getTask(taskId);
      ...
      await db.update(
        `UPDATE workflow_tasks SET 
          ...
          delegated_from = ?
        WHERE id = ?`,
        [targetUser.id, targetUser.name, task.assignee_id, taskId]
      );
  ...
  private async recordTaskHistory(taskId: string, params: {
    action: 'create' | 'assign' | 'claim' | 'complete' | 'transfer' | 'withdraw' | 'cancel' | 'skip' | 'add_signer' | 'delegate' | 'return';
    operator: { id: string; name: string };
