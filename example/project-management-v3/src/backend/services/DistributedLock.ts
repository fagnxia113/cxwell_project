...
export class DistributedLock {
  ...
  async acquire(
    ...
    for (let attempt = 0; attempt < retryCount; attempt++) {
      ...
      const lockId = `${key}-${owner}-${Date.now()}`

      try {
        // 1. 清理该 key 已过期的锁 (原子性保证见后面步骤)
        await db.execute(
          `DELETE FROM workflow_locks WHERE lock_key = ? AND expires_at < ?`,
          [key, now]
        )

        // 2. 尝试插入新锁
        // 注意：workflow_locks.lock_key 必须有 UNIQUE 约束
        try {
          await db.insert(
            `INSERT INTO workflow_locks (id, lock_key, owner, acquired_at, expires_at)
             VALUES (?, ?, ?, ?, ?)`,
            [lockId, key, owner, now, expiresAt]
          )
          ...
          // 如果是主键/唯一约束冲突 (Error Code 1062)，说明锁已被占用
        ...
        console.error(`获取锁异常 (尝试 ${attempt + 1}/${retryCount}):`, error)
        if (attempt < retryCount - 1) {
          await this.sleep(retryDelay)
        }
      }
