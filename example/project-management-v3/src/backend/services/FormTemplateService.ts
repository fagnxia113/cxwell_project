...
class FormTemplateService {
  async initializeDefaultFormTemplates(templates: any[]) {
    for (const template of templates) {
      try {
        ...
        if (existing) {
          ...
          await db.update(
            ...
            [
              ...
              JSON.stringify(template.layout || { type: 'single', columns: 2 }),
          ...
          await this.createTemplate({
            ...
            layout: template.layout || { type: 'single', columns: 2 },
        ...
        console.error(`同步表单模板 ${template.name} 失败:`, error);
      }
    }
  }

  async createTemplate(template: Omit<FormTemplate, 'id' | 'version'> & { key?: string, category?: string }): Promise<FormTemplate> {
    const id = uuidv4();
    ...
    await db.insert(
      `INSERT INTO form_templates (id, \`key\`, name, category, version, layout, fields, sections, style, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        template.key || id,
  ...
  async getTemplateByKey(key: string): Promise<FormTemplate | null> {
    const row = await db.queryOne<any>(
      `SELECT * FROM form_templates WHERE \`key\` = ? OR id = ? OR name = ?`,
      [key, key, key]
    );
  ...
  private parseTemplateRow(row: any): FormTemplate {
    return {
      id: row.id,
      name: row.name,
      version: row.version,
