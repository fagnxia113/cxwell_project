...
export class DefinitionService {
  ...
  async getDefinitionByKey(key: string): Promise<WorkflowDefinition | null> {
    ...
    if (!row && key.startsWith('preset-')) {
      const fallbackKey = key.replace('preset-', '');
  ...
  async suspendDefinition(id: string): Promise<boolean> {
    const definition = await this.getDefinition(id);
    if (!definition) {
      return false;
    }
    ...
    await db.update(
      `UPDATE workflow_definitions SET status = 'suspended' WHERE id = ?`,
      [id]
    );
  ...
  private parseDefinitionRow(row: any): WorkflowDefinition {
    ...
    const serviceTypeMap: Record<string, string> = {
      'create-project': 'createProjectLedger',
      'create-employee': 'createEmployee',
      'service-task-shipping': 'transfer_shipping',
      'service-task-receiving': 'transfer_receiving',
      'service-task-rollback': 'transfer_rollback',
      'create-equipment': 'equipmentInbound',
      'repair-shipping': 'repairShipping',
      'repair-receiving': 'repairReceiving',
      'service-task-scrap-sale': 'scrap_sale',
      'service-task-task-progress': 'task_completion',
      'update-milestones': 'milestone-approval',
      'complete-milestone': 'milestone-completion',
      'do-transfer': 'personnel-project-transfer'
    };

    const convertedNodes = (nodeConfig.nodes || []).map((node: any) => {
      const convertedType = typeMap[node.type] || node.type;
      const convertedNode = {
        ...node,
        type: convertedType
      };
      
      if (convertedType === 'serviceTask' && !node.config?.serviceType) {
        convertedNode.config = {
          ...node.config,
          serviceType: serviceTypeMap[node.id] || node.id
        };
      }
