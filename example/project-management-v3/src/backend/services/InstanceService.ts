...
export class InstanceService {
   ...
   * 获取分布式锁
   */
  async acquireLock(lockKey: string, owner: string, ttlMs: number = 30000): Promise<boolean> {
    ...
    while (retries < LOCK_MAX_RETRIES) {
      try {
        // 清理过期的锁
        await db.execute(
          `DELETE FROM workflow_locks WHERE lock_key = ? AND expires_at < NOW()`,
          [lockKey]
        );
        ...
        await db.insert(
          ...
          [uuidv4(), lockKey, owner, expiresAt]
        );
        return true;
      } catch (error: any) {
  ...
  async releaseLock(lockKey: string, owner: string): Promise<void> {
    await db.execute(
      `DELETE FROM workflow_locks WHERE lock_key = ? AND owner = ?`,
      [lockKey, owner]
    );
  }
  ...
  private static deptMapCache: { data: Record<string, string>, timestamp: number } | null = null;
  private static posMapCache: { data: Record<string, string>, timestamp: number } | null = null;
  private static CACHE_TTL = 60000; // 60s
  ...
  async getInstances(params?: {
    ...
    if (!InstanceService.deptMapCache || now - InstanceService.deptMapCache.timestamp > InstanceService.CACHE_TTL) {
      const departments = await db.query<any>('SELECT id, name FROM departments');
      InstanceService.deptMapCache = {
        data: Object.fromEntries(departments.map((d: any) => [d.id, d.name])),
        timestamp: now
      };
    }

    if (!InstanceService.posMapCache || now - InstanceService.posMapCache.timestamp > InstanceService.CACHE_TTL) {
      const positions = await db.query<any>('SELECT id, name FROM positions');
      InstanceService.posMapCache = {
        data: Object.fromEntries(positions.map((p: any) => [p.id, p.name])),
        timestamp: now
      };
    }

    const deptMap = InstanceService.deptMapCache.data;
    const posMap = InstanceService.posMapCache.data;
  ...
  async updateInstance(id: string, updates: any, operator?: { id: string; name: string }, reason?: string): Promise<boolean> {
    return await db.transaction(async (conn) => {
      ...
      const existing = this.parseInstanceRow(existingRow);
  ...
  async updateVariables(instanceId: string, variables: Record<string, any>): Promise<void> {
    await db.transaction(async (conn) => {
      await this.updateVariablesWithConn(conn, instanceId, variables);
    });
  }

  async updateVariablesWithConn(conn: any, instanceId: string, variables: Record<string, any>): Promise<void> {
    const rows = await conn.query(
      `SELECT * FROM workflow_instances WHERE id = ? FOR UPDATE`,
      [instanceId]
    ) as any[];
    
    // 获取 actual data row (mysql2 might return nested array or object)
    const existingRow = Array.isArray(rows[0]) ? rows[0][0] : rows[0];
    if (!existingRow) {
      throw new Error('流程实例不存在');
    }

    const instance = this.parseInstanceRow(existingRow);
    ...
    for (const [name, value] of Object.entries(variables)) {
      const oldValue = (instance.variables as any)[name];
      if (oldValue !== value) {
        await conn.query(
  ...
  async getVariableHistory(instanceId: string): Promise<any[]> {
    return await db.query(
      `SELECT * FROM workflow_variable_history WHERE instance_id = ? ORDER BY created_at DESC`,
      [instanceId]
    );
  }
