...
export interface ApproverSource {
  type: 'fixed' | 'user' | 'role' | 'department' | 'superior' | 'department_manager' | 'project_manager' | 'warehouse_manager' | 'form_field' | 'field' | 'expression' | 'variable' | 'previous_handler' | 'initiator';
  value: string | string[];
...
export interface WorkflowInstance {
  ...
  end_time?: Date;
  duration?: number;
  result?: 'approved' | 'rejected' | 'withdrawn' | 'terminated' | 'skipped' | 'auto_approved' | 'auto_rejected';
  created_at: Date;
  updated_at: Date;
}
...
export interface WorkflowTask {
  ...
  result?: string;
  comment?: string;
  approval_mode?: 'or_sign' | 'and_sign' | 'sequential' | 'vote';
...
export interface TaskHistory {
  ...
  node_id: string;
  task_name?: string;
  action: 'create' | 'assign' | 'claim' | 'complete' | 'delegate' | 'transfer' | 'withdraw' | 'cancel' | 'skip' | 'add_signer' | 'return';
  operator_id?: string;
