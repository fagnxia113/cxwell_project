...
export class TransferOrderRepository {
    ...
    async createWithItems(
        ...
        const execute = async (client: Prisma.TransactionClient) => {
            ...
            const items = await Promise.all(
                itemsData.map(item => {
                    // Normalize category to match Prisma enum values
                    ...
                    else if (cat.includes('cable') || cat.includes('线')) normalizedCategory = 'cable';
                    else if (cat.includes('accessory') || cat.includes('配件')) normalizedCategory = 'accessory';
                    else if (['instrument', 'fake_load', 'cable', 'accessory'].includes(cat)) normalizedCategory = cat;

                    return client.equipment_transfer_order_items.create({
                        data: {
                            id: item.id,
                            order_id: order.id,
    ...
    async confirmReceiving(
        ...
        receivingData: {
            ...
            isPartial?: boolean
            item_images?: { item_id: string; images: string[] }[]
            item_received_details?: { item_id: string; received_quantity: number }[]
        },
        tx?: Prisma.TransactionClient
    ): Promise<TransferOrder> {
        const client = tx || this.db
        ...
        if (receivingData.item_received_details && receivingData.item_received_details.length > 0) {
            receivingData.item_received_details.forEach(item => {
                updatePromises.push(
                    client.equipment_transfer_order_items.update({
        ...
        if (receivingData.item_images && receivingData.item_images.length > 0) {
            receivingData.item_images.forEach(img => {
                updatePromises.push(
                    client.equipment_transfer_order_items.update({
                        where: { id: img.item_id },
