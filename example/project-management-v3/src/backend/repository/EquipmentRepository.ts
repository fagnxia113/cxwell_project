...
export class EquipmentRepository {
    ...
    async findAll(params?: {
        ...
        if (merge) {
            ...
            const rawSql = `
                SELECT 
                    MAX(e.id) as id,
                    e.equipment_name, e.model_no, e.category, e.brand, e.manufacturer, e.unit,
                    SUM(e.quantity) as quantity,
                    e.health_status, e.usage_status, e.location_status, e.location_id,
                    e.purchase_date,
                    MAX(e.purchase_price) as purchase_price,
                    GROUP_CONCAT(e.manage_code) as manage_codes,
                    GROUP_CONCAT(e.id) as instance_ids,
                    'aggregated' as display_type,
                    COALESCE(w.manager_id, p.manager_id, e.keeper_id) as keeper_id
                FROM equipment_instances e
                LEFT JOIN warehouses w ON e.location_id = w.id AND e.location_status = 'warehouse'
                LEFT JOIN projects p ON e.location_id = p.id AND e.location_status = 'in_project'
                WHERE ${this.buildRawWhere(where, 'e.')}
                GROUP BY e.equipment_name, e.model_no, e.health_status, e.usage_status, e.location_status, e.location_id, e.purchase_date
        ...
        if (data.length > 0) {
            ...
            const projectIds = [...new Set(data.filter(item => item.location_status === 'in_project').map(a => a.location_id).filter(Boolean))] as string[]
            
            const [warehouses, projects] = await Promise.all([
                warehouseIds.length > 0 ? this.db.warehouses.findMany({
                    where: { id: { in: warehouseIds } },
                    select: { id: true, manager_id: true }
                }) : Promise.resolve([]),
                projectIds.length > 0 ? this.db.projects.findMany({
                    where: { id: { in: projectIds } },
                    select: { id: true, manager_id: true }
                }) : Promise.resolve([])
            ])

            const locManagerMap = new Map<string, string>()
            warehouses.forEach(w => w.manager_id && locManagerMap.set(w.id, w.manager_id))
            projects.forEach(p => p.manager_id && locManagerMap.set(p.id, p.manager_id))

            data.forEach(item => {
                if (item.location_id && locManagerMap.has(item.location_id)) {
                    item.keeper_id = locManagerMap.get(item.location_id) || item.keeper_id
                }
            })
        }
    ...
    private buildRawWhere(where?: Prisma.equipment_instancesWhereInput, prefix: string = ''): string {
        if (!where) return '1=1'
        const parts: string[] = ['1=1']
        if (where.category) parts.push(`${prefix}category = '${where.category}'`)
        if (where.location_id) parts.push(`${prefix}location_id = '${where.location_id}'`)
        if (where.location_status) parts.push(`${prefix}location_status = '${where.location_status}'`)
        // 简单处理，实际可根据需要扩充
