...
export default function CustomerListPage() {
  ...
  const stats = useMemo(() => ({
    ...
    recent: Math.floor(customers.length * 0.2)
  }), [customers])

  const filteredCustomers = useMemo(() => {
    return customers.filter(c => {
      return c.name.toLowerCase().includes(keyword.toLowerCase()) ||
        (c.customerNo && c.customerNo.toLowerCase().includes(keyword.toLowerCase()))
    })
  }, [customers, keyword])
              ...
              >
                ...
                <div className="col-span-1">
                  <span className="text-[11px] font-mono font-black text-primary/40 tracking-tighter group-hover:text-primary transition-colors uppercase">
                    {customer.customerNo?.replace('CUST-', '') || idx + 100}
                  </span>
                </div>

                {/* Name */}
                <div className="col-span-4 flex items-center gap-4">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center font-bold text-white shadow-sm bg-primary transition-transform group-hover:rotate-6">
