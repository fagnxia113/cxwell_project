...
export const workflowApi = {
  ...
  // 启动流程 (正式发起)
  startProcess: (data: { flowCode: string; businessId?: string; variables?: any }) =>
  ...
  // 办理任务 (通过/退回等)
  submitTask: (taskId: string, action: 'pass' | 'reject', variables: any = {}, message: string = '', targetNodeCode?: string) =>
  ...
  // 直接驳回接口
  reject: (taskId: string, comment: string, targetNodeCode?: string) =>
    apiClient.post<any>('/api/workflow/tasks/reject', { taskId, comment, targetNodeCode }),
};
