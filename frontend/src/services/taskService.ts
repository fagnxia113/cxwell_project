...
export const taskService = {
  async myTasks(params?: { status?: string[] }): Promise<Task[]> {
    const res = await apiClient.get<any>('/api/workflow/tasks/todo', { params })
    return res.data || []
  },

  async getTaskDetail(id: string): Promise<Task> {
    const res = await apiClient.get<any>(`/api/workflow/tasks/${id}`)
    return res.data || res
  },

  async completeTask(id: string, params: CompleteTaskParams): Promise<void> {
    return apiClient.post(`/api/workflow/tasks/${id}/submit`, params)
  },
