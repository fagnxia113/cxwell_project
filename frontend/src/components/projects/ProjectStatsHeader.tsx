...
function StatCard({ icon: Icon, label, value, subValue, color, delay }: any) {
  ...
  return (
    <motion.div 
      ...
      transition={{ delay }}
      className={`bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3 group transition-all ${colorConfig[color]}`}
    >
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform ${colorConfig[color].split(' ')[0]} ${colorConfig[color].split(' ')[1]}`}>
        <Icon size={18} />
      </div>
      <div className="space-y-0">
        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{label}</p>
        <div className="flex items-baseline gap-1">
          <span className="text-lg font-black text-slate-800 tabular-nums tracking-tighter">{value}</span>
          {subValue && <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">{subValue}</span>}
        </div>
      </div>
    </motion.div>
  )
